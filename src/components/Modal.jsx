                ...
                <div className="p-3">
                    ...
                    <div className="mb-8 w-fit h-fit">
                        <div className="flex justify-between">
                            <div></div>
                            <button className="border border-gray-800 rounded-full hover:bg-gray-200 p-1" onClick={() => navigate(-1, {replace: true})}>
                    ...
                    <div className="flex justify-center items-center gap-6 mb-4">
                        <button className="text-xl bg-gray-800 border border-gray-800 text-white rounded-3xl font-semibold p-2 px-8">
                            <Link 
                                to={action.post_redirection ? `/signin/?post=${window.location.pathname.split('/')[2]}` : '/signin'}
                                onClick={() => navigate(action.post_redirection ? `/signin/?post=${window.location.pathname.split('/')[2]}` : '/signin', { replace: true })}
                                reloadDocument
                                preventScrollReset
                                >SignIn</Link>
                        </button>
                        <button className="text-xl bg-white text-gray-800 border border-gray-800 rounded-3xl font-semibold p-2 px-8">
                            <Link
                                to={action.post_redirection ? `/signup/?post=${window.location.pathname.split('/')[2]}` : '/signup'}
                                onClick={() => navigate(action.post_redirection ? `/signup/?post=${window.location.pathname.split('/')[2]}` : '/signup', { replace: true})}
                                reloadDocument
                                preventScrollReset
                                >SignUp</Link>
                        </button>
